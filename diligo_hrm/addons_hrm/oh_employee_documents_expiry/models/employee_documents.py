...
class HrEmployeeDocument(models.Model):
        ...
        for each in self:
            if each.expiry_date:
                ...
                if exp_date < date.today():
                    raise UserError("Tài liệu của bạn đã hết hạn.")

    @api.model
    def _default_code(self):
        last_id = self.search([], order='id desc', limit=1).id
        last_id += 1
        code = 'DX' + str(last_id).zfill(6)
        return code

    name = fields.Char(string='Mã tài liệu', required=True, default=lambda self: self._default_code(), help='You can give your'
                                                                                 'Document number.')
    description = fields.Text(string='Description', copy=False, help="Description")
    expiry_date = fields.Date(string='Ngày hết hạn', copy=False, help="Date of expiry")
    ...
    issue_date = fields.Date(string='Ngày phát hành', default=fields.datetime.now(), help="Date of issue", copy=False)
    document_type = fields.Many2one('document.type', string="Loại tài liệu", help="Document type")
