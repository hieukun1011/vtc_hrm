...
class Document(models.Model):
    ...
    document_user = fields.Many2one('res.users', string='appro document')
    ...
    document_text = fields.Selection([('notify', '[TB] Notify'),
                              ('incomingtext', '[VBT] Incoming text'),
                              ('documenttranfer', '[VBD] Document transferred'),
                              ('internaltext', '[VBNB] Internal text')],
                             default='notify', string='Document text')
    doc_code = fields.Char(string='Code document')

    @api.onchange(document_text)
    def onchange_document_text(self):
        if self.document_text:
            sequence = self.env['ir.sequence'].next_by_code('diligo_document_code_action')
            if self.document_text == 'notify':
                self.doc_code = 'TB' + str(sequence)
            elif self.document_text == 'incomingtext':
                self.doc_code = 'VBT' + str(sequence)
            elif self.document_text == 'documenttranfer':
                self.doc_code = 'VBD' + str(sequence)
            else:
                self.doc_code = 'VBNB' + str(sequence)
    ...
    def promulgate_document(self):
        self.state = 'promulgate'

    def closed_document(self):
        self.state = 'closed'
