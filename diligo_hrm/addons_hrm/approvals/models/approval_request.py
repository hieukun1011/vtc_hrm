...
class ApprovalRequest(models.Model):
    ...
    resource = fields.Selection([
        ('laggermeeting', 'Lagger meeting'), ('smallmeeting', 'Small meeting')], string="Resource")
    summary = fields.Html(string="Summary")
    business_trip_results = fields.Html(string="Business trip results")
    vehicle = fields.Selection([
        ('coach', 'Coach'),
        ('companycar', 'Company car'),
        ('plan', 'Plan'),
        ('railwaytrain', 'Railway train'),
        ('motobike', 'Motobike'), ('othermeans','Other means')], string="Vehicle", default='companycar')
    intended_use = fields.Selection([
        ('personal', 'Personal'), ('office', 'Office')], string="Intended use")
    proposed_date = fields.Datetime(string='Proposed date', default=lambda self: fields.Datetime.now())
    time_need = fields.Date(string='Time need')
    department = fields.Many2one('hr.department',  string="department ") #related='employee_id.department_id',

    has_date_start = fields.Selection(related="category_id.has_date_start")
    ...
    has_time_need = fields.Selection(related="category_id.has_time_need")
    has_proposed_date = fields.Selection(related="category_id.has_proposed_date")
    has_intended_use = fields.Selection(related="category_id.has_intended_use")
    has_vehicle = fields.Selection(related="category_id.has_vehicle")
    has_resource = fields.Selection(related="category_id.has_resource")
    ...
    def onchange_request_owner_id(self):
        if self.request_owner_id.employee_id:
            self.department = self.request_owner_id.employee_id.department_id.id
