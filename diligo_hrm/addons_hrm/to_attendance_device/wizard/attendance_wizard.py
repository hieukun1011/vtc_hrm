        ...
        for activity_id in activity_ids:
            ...
            for att in unsync_data:
                ...
                if att.type == 'checkout':
                    ...
                    date_attendance = att.timestamp.astimezone(tz).date()
                    last_att = HrAttendance.search(
                        [('employee_id', '=', employee_id.id), ('date_attendance', '=', date_attendance)], limit=1)

                    if last_att:
                        if last_att.check_in > att.timestamp:
                            last_att.with_context(synch_ignore_constraints=synch_ignore_constraints).write({
                                'check_in': att.timestamp,
                                'checkin_device_id': att.device_id.id,
                                'check_out': last_att.check_in,
