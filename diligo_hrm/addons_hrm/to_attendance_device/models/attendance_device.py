...
class AttendanceCheck(models.Model):
    ...
    device_uid_count = fields.Integer(string='Uid')
...
class AttendanceDevice(models.Model):
    ...
    auto_clear_attendance_error_notif = fields.Boolean(string='Auto Clear Attendance Notif.', default=True,
                                                        oldname='auto_clear_atttendance_error_notif', tracking=True,
    ...
    def action_attendance_download(self):
        ...
        for r in self:
            ...
            attendance_check = self.env['attendance.check'].sudo().search([('device_id', '=', r.id)], limit=1)
            start_d = datetime.now() - timedelta(days=30)
            end_d = datetime.now() + timedelta(days=7)
            dt = attendance_check.timestamp if attendance_check.timestamp else start_d
            flag = False
            for attendance in attendance_data:
                utc_t = r.convert_time_to_utc(attendance.timestamp, r.tz)
                utc_d = r.convert_time_to_utc(datetime.combine(attendance.timestamp.date(), datetime.min.time()), r.tz)
                utc_d = utc_d.replace(tzinfo=None)
                utc_timestamp = utc_t.replace(tzinfo=None)
                if dt <= utc_timestamp < end_d:
                    ...
                    if user_id:
                        ...
                        if not duplicate_attend:
                                try:
                                    valid = True if user_id.employee_id else False
                                    check_punch = DeviceUserAttendance.search([
                                        ('user_id', '=', user_id.id),
                                        ('timestamp', '>=', utc_d),
                                        ('timestamp', '<', utc_timestamp)], limit=1)
                                    punch = 1 if check_punch else 0
                                    vals = {
                                        'device_id': r.id,
                                        'user_id': user_id.id,
                                        'timestamp': utc_timestamp,
                                        'status': punch,
                                        'attendance_state_id': attendance_states[punch],
                                        'valid': valid,
                                        }
                                    vals2 = {
                                        'device_id': r.id,
                                        ...
                                        'device_uid_count': attendance.uid
                                    }
                                    flag = True
                                    ...
                                    error_msg += _("Error create DeviceUserAttendance record: device_id %s; user_id %s; attendance_state_id %s.<br />") % (
                                        r.id,
                                        user_id.id,
                                        # mail_template.format_tz(r.env, attendance.timestamp, r.tz),
                                        )
                                    _logger.error(error_msg)
                                    pass
            if flag:
                if attendance_check:
                    self.env['attendance.check'].sudo().update(vals2)
                else:
                    self.env['attendance.check'].sudo().create(vals2)
    ...
    def action_device_information(self):
        ...
        for r in self:
            try:
                r.ensure_one()
                force_udp = r.protocol == 'udp'
                password = r.password or 0
                cached_key = r.protocol + str(r.omit_ping) + str(r.timeout) + str(password)
                if r.ip:
                    cached_key += r.ip
                if r.port:
                    cached_key += str(r.port)
                if not cached_key in r.zk_cache.keys() or self.env.context.get('no_zk_cache', False):
                    r.zk_cache[cached_key] = ZK(r.ip, r.port, r.timeout, password=password,
                                                   force_udp=force_udp,
                                                   ommit_ping=r.omit_ping)
                zk = r.zk_cache[cached_key]
