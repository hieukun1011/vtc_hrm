...
class HrAnnouncements(models.Model):
    ...
    def _announcement_count(self):
        for obj in self:
            announcement_ids = self.env['hr.announcement'].sudo().search(['|', '|', ('employee_ids', 'in', [obj.id]),
                                                                          ('is_announcement', '=', True),
                                                                          ('department_ids', 'in', self.department_id.id),
                                                                          ('state', 'in', ('approved', 'done'))])
            test = self.env['hr.announcement'].sudo().search([('department_ids', 'in', self.department_id.id)])
