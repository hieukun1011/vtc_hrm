...
class HrAnnouncementTable(models.Model):
    ...
    name = fields.Char(string='Số thông báo:')
    ...
    company_id = fields.Many2one('res.company', string='Công ty', states={'draft': [('readonly', False)]}, readonly=True,
                                 default=lambda self: self.env.user.company_id)
    is_announcement = fields.Boolean(string='Thông báo chung?')
    announcement_type = fields.Selection([('employee', 'Nhân viên'), ('department', 'Phòng ban'), ('job_position', 'Vị trí công việc')],
                                         string="Loại thông báo", states={'draft': [('readonly', False)]}, readonly=True)
    employee_ids = fields.Many2many('hr.employee', 'hr_employee_announcements', 'announcement', 'employee', domain="[('company_id', '=', company_id)]",
                                    string='Nhân viên')
    department_ids = fields.Many2many('hr.department', 'hr_department_announcements', 'announcement', 'department', domain="[('company_id', '=', company_id)]",
                                      string='Phòng ban')
    ...
    date_start = fields.Date(string='Ngày bắt đầu', default=fields.Date.today(), required=True, states={'draft': [('readonly', False)]}, readonly=True)
    date_end = fields.Date(string='Ngày kết thúc', default=fields.Date.today(), required=True, states={'draft': [('readonly', False)]}, readonly=True)
