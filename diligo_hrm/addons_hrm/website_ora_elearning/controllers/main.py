...
class WebsiteSlidesORA(WebsiteSlides):
    ...
    def slides_channel_home(self, **post):
        ...
        channel_ht = request.env['slide.channel'].search([('department_id', '=', 'ht')] + domain)
        channel_ptsp = request.env['slide.channel'].search([('department_id', '=', 'ptsp')] + domain)
        channel_ccu = request.env['slide.channel'].search([('department_id', '=', 'ccu')] + domain)
        channel_nm = request.env['slide.channel'].search([('department_id', '=', 'nm')] + domain)
        channel_mh = request.env['slide.channel'].search([('department_id', '=', 'mh')] + domain)
        channel_kv = request.env['slide.channel'].search([('department_id', '=', 'kv')] + domain)
        channel_khcc = request.env['slide.channel'].search([('department_id', '=', 'khcc')] + domain)
        ...
        values.update({
            ...
            'channel_ht': channel_ht,
            'channel_ptsp': channel_ptsp,
            'channel_ccu': channel_ccu,
            'channel_nm': channel_nm,
            'channel_mh': channel_mh,
            'channel_kv': channel_kv,
            'channel_khcc': channel_khcc,
        ...
        return request.render('website_ora_elearning.courses_home_inherit', values)
