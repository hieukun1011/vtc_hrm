...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            day_from = user_tz.localize(day_from, is_dst=None)
            day_from = day_from.astimezone(utc)
            day_to = user_tz.localize(day_to, is_dst=None)
            day_to = day_to.astimezone(utc)
            work_day = self.env['hr.attendance'].search([('check_in', '>=', day_from),
                                                             ('check_out', '<=', day_to),
                                                             ('employee_id', '=', contract.employee_id.id),
                                                             ('state', '=', 'validate')])
            number_of_days = len(work_day)
            w_d = {
                'name': 'Công thực tế',
                'sequence': 2,
                'code': 'WORKDAY',
                'number_of_days': number_of_days,
                'number_of_hours': sum(member.worked_hours_real for member in work_day),
                'contract_id': contract.id,
            }
