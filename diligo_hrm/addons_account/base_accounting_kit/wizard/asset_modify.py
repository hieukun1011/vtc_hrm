...
class AssetModify(models.TransientModel):
    ...
    value_residual = fields.Float(string="Depreciable Amount", help="New residual amount for the asset", readonly=True)
    salvage_value = fields.Float(string="Not Depreciable Amount", help="New salvage amount for the asset", readonly=True)
    depreciation_value = fields.Float('Depreciation value')
    loss_value = fields.Float('Loss value')
    ...
    def default_get(self, fields):
        ...
        if 'salvage_value' in fields:
            res.update({'salvage_value': asset.salvage_value})
    ...
    def modify(self):
        ...
        old_values = {
            ...
            'depreciation_value': self.depreciation_value + asset.depreciation_value,
            'loss_value': self.loss_value + asset.loss_value,
        ...
        asset_vals = {
            ...
            'loss_value': self.loss_value + asset.loss_value,
            'salvage_value': self.salvage_value,
            'description': asset.description + '</br>' + self.name if asset.description else self.name,

        }
        asset.write(asset_vals)
        asset.compute_depreciation_board()
        asset.history += self.env['history.asset.modify'].create({
