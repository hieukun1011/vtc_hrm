...
class Asset_Department(models.Model):
    ...
    parent_id = fields.Many2one('asset.department', string='Parent Department', index=True, domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    child_ids = fields.One2many('asset.department', 'parent_id', string='Child Departments')
...
class AccountAssetCategory(models.Model):
    ...
    category_property = fields.Char(string='Category property', required=True)

    @api.constrains('code_type')
    def _constraints(self):
        if not self.code_type:
            self.code_type = str(self.category) + str(self.category_property)
...
class AccountAssetAsset(models.Model):
    ...
    code = fields.Char(string='Reference', size=32, readonly=True,
                       states={'draft': [('readonly', False)]})
    tag_number = fields.Char(string='Tag number', size=32, readonly=True)
    department_id = fields.Many2one('asset.department', 'Department')
    ...
    date_asset = fields.Date(string='Asset increase day ', required=True, readonly=True,
                       states={'draft': [('readonly', False)]},
                       default=fields.Date.context_today)
    ...
    value_residual = fields.Float(compute='_amount_residual',
                                  digits=0, string='Residual Value', tracking=True)
    method_time = fields.Selection(
        [('number', 'Number of Entries'), ('end', 'Ending Date')],
    ...
    salvage_value = fields.Float(string='Salvage Value', digits=0,
                                 readonly=True,
                                 states={'draft': [('readonly', False)]},
                                 help="It is the amount you plan to have that you cannot depreciate.", tracking=True)
    ...
    device_line_ids = fields.One2many('sci.device.main', 'asset_id', string='Device Lines')
    account_asset_id = fields.Many2one('account.account', string='Fixed Asset Account',
                                       help="Account used to record the purchase of the asset at its original price.",
                                       store=True,
                                       states={'draft': [('readonly', False)], 'model': [('readonly', False)]},
                                       domain="[('company_id', '=', company_id), ('is_off_balance', '=', False)]")
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Account', readonly=True,
                                              states={'draft': [('readonly', False)], 'model': [('readonly', False)]},
                                              domain="[('internal_type', '=', 'other'), ('deprecated', '=', False), ('company_id', '=', company_id), ('is_off_balance', '=', False)]",
                                              help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Expense Account', readonly=True,
                                                      states={'draft': [('readonly', False)],
    ...
    depreciation_value = fields.Float('Depreciation value', readonly=True, tracking=True)
    loss_value = fields.Float('Loss value', readonly=True, tracking=True)
    description = fields.Text('Description', tracking=True)
    history = fields.One2many('history.asset.modify', 'asset_id', string='History')

    @api.onchange('category_id')
    def _onchange_cate_code(self):
        if self.category_id:
            sequence = self.env['ir.sequence'].next_by_code('account_cate_code_action')
            if self.category_id:
                self.tag_number = str(self.department_id.value) + '.' + str(self.category_id.code_type) + '.' + str(sequence)

    @api.onchange('department_id')
    def _onchange_code(self):
        if self.department_id:
            sequence = self.env['ir.sequence'].next_by_code('account_asset_code_action')
            if self.department_id:
                self.code = str(self.department_id.value) + '.' + str(sequence)

    @api.onchange('category_id')
    def _onchange_account_asset_id(self):
        if self.category_id:
            self.account_asset_id = self.category_id.account_asset_id
            self.account_depreciation_id = self.category_id.account_depreciation_id
            self.account_depreciation_expense_id = self.category_id.account_depreciation_expense_id
        ...
        if self.value_residual != 0.0:
            ...
            for x in range(len(posted_depreciation_line_ids),
                ...
                vals = {
                    ...
                    'depreciated_value': self.value_residual - residual_amount,
    ...
    def validate(self):
        ...
        for item in range(self.quantity):
            data_recheck.append((0, 0, {
                'name': self.name,
                'default_code': self.code + str(item),
                'serial_no': self.tag_number,
                'first_date_use': self.date,
            }))
        self.device_line_ids = data_recheck
    ...
    @api.depends('value', 'salvage_value', 'depreciation_line_ids.move_check',
                 'depreciation_line_ids.amount', 'depreciation_value')
    def _amount_residual(self):
        for record in self:
            ...
            record.value_residual = record.value - total_amount - record.salvage_value + record.depreciation_value - record.loss_value
...
class AccountAssetDepreciationLine(models.Model):
    ...
    def create_move(self, post_move=True):
        ...
        for line in self:
            ...
            move_line_2 = {
                'name': asset_name,
                'account_id': line.asset_id.account_depreciation_expense_id.id,
...
class HistoryAssetModify(models.Model):
    _name = 'history.asset.modify'

    asset_id = fields.Many2one('account.asset.asset', string='Asset')
    value_residual = fields.Integer('Value residual')
    salvage_value = fields.Integer('Salvage value')
    depreciation_value = fields.Integer('Depreciation value')
    loss_value = fields.Integer('Loss value')
    method_number = fields.Integer('Method number')
    method_period = fields.Integer('Method period')
