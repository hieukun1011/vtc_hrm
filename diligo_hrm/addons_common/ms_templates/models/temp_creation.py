...
class TemplateCreation(models.Model):
    ...
    def _get_file_extension(self):
        for item in self:
            if item.template_name:
                item.template_extension = str(item.template_name).rsplit('.', 1)[1]
            else:
                item.template_extension = 'VNWIVBNWEIU'

    @api.constrains('template_name')
    def _constrain_extension(self):
        for item in self:
            if not item.template or item.template_extension not in ['doc', 'docx', 'xls', 'xlsx']:
    ...
    def _constrain_excel_cell(self):
        for item in self:
            if item.template_extension in ['xls', 'xlsx']:
                if item.max_col and not item.max_col.isalpha():
                    raise ValidationError(_("Hide to col's value must be string."))
                if item.grp_by_cell and not item.grp_by_cell.strip().rstrip('0123456789').isalpha():
                    raise ValidationError(_("Invalid 'Group by - start cell' value format."))
                if item.current_datetime_positions:
                    for cell_position in item.current_datetime_positions.split(','):
                ...
                if item.current_datetime_positions_2:
                    for cell_position in item.current_datetime_positions_2.split(','):
                        if not cell_position.strip().rstrip('0123456789').isalpha():
                            raise ValidationError(_("Invalid 'Current datetime position' value format."))
    ...
    def write(self, vals):
        for item in self:
            ...
            if item.wizard_action:
                if vals.get('access_groups'):
                    item.wizard_action.groups_id = [(6, 0, item.access_groups.ids)]
                if vals.get('name'):
                    item.wizard_action.name = item.name
        return res
    ...
    def export_excel(self, records, **kwargs):
        self.ensure_one()
        ...
        # format mixin_list to export record with grp_by_fields: [grp_by_value_1, record 1, record 2, value 2, record 3, record 4]
            ...
            sub_group_start_row = grp_start_row  # for group expanding
            sub_group_start_col = grp_start_col  # for group expanding
        ...
        for num, ws in enumerate(working_ws):
            ...
            if self.grp_by_field:
                ws.sheet_properties.outlinePr.summaryBelow = False
            mixin_list = mixin_list if self.all_in_one else records[num] if len(records) > 0 else []
            ...
            for num2, item in enumerate(mixin_list):
                if not isinstance(type(item), models.MetaModel) and self.grp_by_field:  # if item is a grp_by_field value
                    if num2 > 0:
                    ...
                    if self.export_horizontally:
                        ...
                        if sub_group_start_col > grp_start_col:
                            ws.column_dimensions.group(start=sub_group_start_col, end=grp_start_col + distance - 1, hidden=False)
                        sub_group_start_col = grp_start_col + distance + 1
                    else:
                        ws.cell(grp_start_row + distance, grp_start_col).value = item
                        if sub_group_start_row > grp_start_row:
                            ws.row_dimensions.group(start=sub_group_start_row, end=grp_start_row + distance - 1, hidden=False)
                        sub_group_start_row = grp_start_row + distance + 1
                else:
            ...
            if max_cell > 0:
                if self.export_horizontally:
                    if self.grp_by_field:
                        ws.column_dimensions.group(start=get_column_letter(sub_group_start_col), end=get_column_letter(max_cell-2), hidden=False)
                    if max_cell < column_index_from_string(self.max_col):
                        ws.column_dimensions.group(start=get_column_letter(max_cell), end=self.max_col, hidden=True, outline_level=0)
                elif not self.export_horizontally:
                    if self.grp_by_field:
                        ws.row_dimensions.group(start=sub_group_start_row, end=max_cell - 2, hidden=False)
                    if max_cell < self.max_row:
                        ws.row_dimensions.group(start=max_cell, end=self.max_row, hidden=True, outline_level=0)

            # external keywords part
            if kwargs:
    ...
    def export_records_data(self, records, **kwargs):
        self.ensure_one()
        if self.template_extension in ['xls', 'xlsx']:
            return self.export_excel(records, **kwargs)
    ...
    def sample_report(self):
        self.env['ir.attachment'].create({
            ...
            'datas': self.export_records_data(self.env[self.main_model.model].search([], limit=self.max_samples or 5)),
...
class ChosenFields(models.Model):
    ...
    # @api.one
    @api.constrains('replacement_value')
    def _constrain_replacement_value(self):
        for replacement_value in self:
            replacement_dict = {}
            try:
                replacement_dict = literal_eval(replacement_value)
            except:
                raise ValidationError(_('Wrong format for replacement value.'))
            if replacement_dict:
                for key, value in replacement_dict.items():
                    if not isinstance(key, pycompat.string_types) or not isinstance(value, pycompat.string_types):
                        raise ValidationError(_('Replacement value only accept string'))
    ...
    def _get_value(self):
        for item in self:
            if item.line.main_model:
                records = self.env[item.line.main_model.model].search([], limit=item.line.max_samples or 3)
                list_value = item.line.get_field_value(records, item.python_code, item.replacement_format)
                item.field_value = ', '.join(str(i) for i in list_value)

    @api.depends('main_field')
    def _get_sub_model(self):
        for item in self:
            if item.main_field.ttype in ['many2one', 'one2many', 'many2many']:
                item.sub_model = self.env['ir.model'].search([('model', '=', item.main_field.relation)])
            else:
                item.sub_model = False
